module JsonErrors
    ...
    included do
      ...
      def render_400(errors = 'required parameters invalid')
        render_errors(errors, 400, '400')
      end
  
      def render_401(errors = 'unauthorized access')
        render_errors(errors, 401, '401')
      end
  
      def render_404(errors = 'not found')
        error_message = "Record not found: #{errors.model.underscore}"
        render_errors(error_message, 404, '404')
      end
  
      def render_422(errors = 'could not save data')
        render_errors(errors, 422, '422')
      end
  
      def render_500(errors = 'internal server error')
        render_errors(errors, 500, '500')
      end
  
  
      def render_errors(errors, status = 400, error_code)
        data = {
